...
```text
Ghostty = 탭 + 분할 + 윈도우를 하나의 터미널에서 관리
     ↓
⌘T (새 탭) / ⌘N (새 창) / ⌘D (좌우 분할) / ⌘⇧D (상하 분할)
     ↓
⌘[ / ⌘] (분할 간 이동) / ⌘⌥방향 (포커스 이동)
```
...
| 도구 | 관계 |
|------|------|
| tmux | SSH 세션 유지 시 ghostty 안에서 tmux 사용 |
| zsh | ghostty의 기본 셸 |
| Ghostty | 자체 탭/분할로 tmux 없이도 사용 가능 |
...
> **색이 안 바뀔 때:** ① `+show-config`/`+list-themes`로 파일 유효성 먼저 증명 → ② 유효하면 `⌘ ⇧ ,` 리로드(Ghostty는 시작 시 1회만 읽음, 자동 감지 X) → ③ 그래도면 tmux가 자체 팔레트로 덮는 것일 수 있으니 tmux 밖 새 창(`⌘ N`)에서 확인.
