...
| `<leader>gs` | Git Status (수정된 파일 picker) | picker |
| `<leader>gd` | Git Diff (hunks) | picker |
| `<leader>gD` | Git Diff (origin) | picker |
| `<leader>gS` | Git Stash | picker |

> `<leader>gs`는 `fg`(git tracked 전체)와 달리 **변경/스테이지된 파일만** 추린다. 위 4개는 기본 picker(snacks) 제공.
