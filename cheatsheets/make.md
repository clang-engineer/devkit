...
```text
Makefile → 타깃(target) → 의존성(dependencies) → 명령(commands)
              ↓
        .PHONY (가짜 타깃)
        %: (패턴 룰)
        $(shell) (셸 명령 호출)
...
|------|------|------|
| 빌드 | `make` | 기본 타깃 실행 |
| 특정 타깃 | `make <target>` | 지정 타깃 실행 |
| 정리 | `make clean` | 생성된 파일 삭제 |
| 변수 | `VAR := value` | 변수 정의 |
| 패턴 | `%.o: %.c` | 패턴 룰 |
...
| 도구 | 관계 |
|------|------|
| docker | `make docker-build` 같은 자동화 |
| git | `make deploy` 같은 배포 자동화 |
| shell | Makefile 안에서 셸 명령 실행 |
