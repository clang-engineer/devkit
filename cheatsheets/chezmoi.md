...
- **`.chezmoiignore`** — source 루트에 두면 그 패턴은 target에서 제외 (템플릿 지원 → OS별 무시 가능). `README.md` 등 리포에는 두되 배포 안 할 파일에.
...
- age 키 생성: `chezmoi age-keygen`. edit는 `chezmoi edit`(자동 복호화) 또는 `chezmoi edit-encrypted`.
