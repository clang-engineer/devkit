...
| 단계 | 명령 | 설명 |
|------|------|------|
| 이름으로 PID | `pgrep -fl <pattern>` | 프로세스 찾기 |
| 이름으로 종료 | `pkill -f <pattern>` | 프로세스 종료 |
| 포트 확인 | `lsof -iTCP:<port> -sTCP:LISTEN` | 포트 사용 프로세스 |
| 시그널 전송 | `kill -<번호> <pid>` | 종료/중지/리로드 |
...
> 시그널 번호·이름·동작은 위 [시그널 번호 표](#시그널-번호-표) 참조.
