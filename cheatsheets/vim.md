...
| 단계 | 명령 | 설명 |
|------|------|------|
| 입력 모드 | `i` / `a` / `o` | 커서 위치/다음/아래 줄 |
| Normal 복귀 | `Esc` / `Ctrl+[` | 어떤 모드에서든 |
| 저장/종료 | `:wq` / `:q!` | 저장 후 종료/강제 종료 |
| 변경 취소 | `u` / `Ctrl+R` |_undo/redo_ |
| 검색 | `/패턴` | `n`/`N`로 다음/이전 |
...
| 도구 | 관계 |
|------|------|
| LazyVim | Vim 기반 사전 설정 배포 |
| Neovim | Lua 기반 모던 Vim |
| VS Code | Vim 확장으로 vim 키맵 사용 |
| ripgrep | `:grep` 명령 내부 사용 |
